...
(async () => {
  ...
  for (const [pais, desde, hasta, nombre] of casos) {
    ...
    // La linea que reconcilia con Zoho: «siguen como Lead» + «ya son Trato»
    // tienen que dar el total, y los convertidos tienen que ser EXACTAMENTE la
    // suma de los «Calificados» de los dos embudos mas los otros canales. Es la
    // causa numero uno de que el tablero «no cuadre» con una vista del CRM, asi
    // que el texto que lo explica es un test.
    // El patron pide que EMPIECE en digito: `[\d.,]+` pelado tambien casa con
    // la coma y el punto de la prosa, y la prueba termina comparando puntuacion.
    const z = await p.evaluate(() => {
      const o = {};
      document.querySelectorAll("#embudos-zoho b[data-z]").forEach(e => {
        if (!(e.dataset.z in o)) o[e.dataset.z] = Number(e.textContent.replace(/[.,]/g, ""));
      });
      return o;
    });
    const eZ = await p.evaluate(([pais, desde, hasta]) => {
      const D = JSON.parse(document.getElementById("datos").textContent);
      const V = D.dic, F = V.fecha;
      const l = D.leads.filter(r => F[r[0]] >= desde && F[r[0]] <= hasta
        && (!pais || V.pais[r[1]] === pais));
      return { total: l.length, conv: l.filter(r => r[11] === 1).length,
               cal: l.filter(r => r[7] === 1).length };
    }, [pais, desde, hasta]);
    ok(z.total === e.leads, "la nota de Zoho parte del total de la ventana",
       z.total + " vs " + e.leads);
    ok(z.conv === eZ.conv, "los convertidos salen de la marca del CRM, no de tener Trato",
       z.conv + " vs " + eZ.conv);
    ok(z.conv + z.sigue === e.leads, "convertidos + los que siguen dan el total",
       z.conv + " + " + z.sigue + " = " + e.leads);
    ok(z.cal === eZ.cal, "y los que SI tienen Trato son los «Calificados»",
       z.cal + " vs " + eZ.cal);
    ok((z.huerfanos || 0) === eZ.conv - eZ.cal,
       "los convertidos sin Trato se declaran, no se esconden",
       (z.huerfanos || 0) + " vs " + (eZ.conv - eZ.cal));
    // Los convertidos nunca pueden ser menos que los que tienen Trato: si pasa,
    // alguien cruzo las dos marcas.
